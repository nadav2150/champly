...
import type { StoreProductOption } from './create-store-modal';

type ZoneRow = { clientKey: string; id?: string; name: string };
...
export function EditStoreModal({
  ...
  useEffect(() => {
    ...
    setSelectedProductIds(new Set(store.productIds));
    setProductSearch('');
    setClientError(null);
  }, [open, store]);
  ...
  function handleFinalSubmit(e: React.FormEvent) {
    ...
    fd.set('intent', 'update-store');
    fd.set('id', store.id);
    fd.set('name', name.trim());
    fd.set('address', address.trim());
    const zonesPayload = zoneRows
      .map((z) => ({
        id: z.id,
        name: z.name.trim(),
      }))
      .filter((z) => z.name.length > 0);
    fd.set('zonesJson', JSON.stringify(zonesPayload));
              ...
              <ul className="flex max-h-56 flex-col gap-1 overflow-y-auto rounded-lg border border-white/10 p-2">
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((p) => {
                    ...
                    const isOther =
                      otherId !== null && otherId !== store.id;
                    const otherName =
                      isOther && storeNameById[otherId]
