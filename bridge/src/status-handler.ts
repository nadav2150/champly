...
export async function handleStatusMessage(cfg: BridgeConfig, payload: Buffer): Promise<void> {
  ...
  if (msg.gw !== cfg.gateway.mac) {
    return;
  }

  try {
    await updateGatewayLastSeen(cfg, msg.gw, msg.tm);
  } catch (err) {
